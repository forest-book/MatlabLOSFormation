...
if (clientID>-1)
    ...
    [r, Quad(3)]   = sim.simxGetObjectHandle(clientID3, 'Quadcopter#3', sim.simx_opmode_blocking);
    [r, target(3)] = sim.simxGetObjectHandle(clientID3, 'Quadcopter_target#3', sim.simx_opmode_blocking);

    [r, Quad(4)]   = sim.simxGetObjectHandle(clientID4, 'Quadcopter#5', sim.simx_opmode_blocking);
    [r, target(4)] = sim.simxGetObjectHandle(clientID4, 'Quadcopter_target#5', sim.simx_opmode_blocking);

    [r, Quad(5)]   = sim.simxGetObjectHandle(clientID5, 'Quadcopter#7', sim.simx_opmode_blocking);
    [r, target(5)] = sim.simxGetObjectHandle(clientID5, 'Quadcopter_target#7', sim.simx_opmode_blocking);
    ...
    k0l = zeros(Quad_num-1,2);
    k0l(1,:) = [5,200]; k0l(2,:) = [5,200];
    k0l(3,:) = [5,200]; k0l(4,:) = [5,200];
    ...
    %リーダと各フォロワの距離を規定
    d = zeros(Quad_num-1,formation_num);
    d(:,1) = [320,240,160,80];
    d(:,2) = [200,200,100,100]; 
    ...
    Q.Cin = zeros(loop_num+1,Quad_num);
    ...
    %リーダの初期座標を設定
    Q.Coord(1,1,1)=-400; Q.Coord(2,1,1)=0; Q.Coord(3,1,1)=220;
    
    %フォロワ1,2,3,4の初期座標を設定        
    Q.Coord(1,1,2)=-420; Q.Coord(2,1,2)=-110; Q.Coord(3,1,2)=250;
    Q.Coord(1,1,3)=-500; Q.Coord(2,1,3)=-60;  Q.Coord(3,1,3)=250;
    Q.Coord(1,1,4)=-520; Q.Coord(2,1,4)= 45;   Q.Coord(3,1,4)=250;
    Q.Coord(1,1,5)=-600; Q.Coord(2,1,5)=-110; Q.Coord(3,1,5)=250;
...
　　　　　　　クワッドロータの障害物センサの値を利用する
        ...
        if norm(G_l) > 30
            ...
            for i = 1:Quad_num-1
                ...
                new_aixs = axis_transform(Q.speed_dir(:,loop,Lnumber));
                %%記録用
